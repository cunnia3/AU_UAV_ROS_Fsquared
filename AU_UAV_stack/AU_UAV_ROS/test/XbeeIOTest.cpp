...
void convertMavlinkTelemetryToROS(mavlink_au_uav_t *mavMessage, AU_UAV_ROS::TelemetryUpdate *tUpdate) {
...
TEST(XbeeIOTest, openPort1){
  std::string port = "/dev/ttyUSB0";
  int fd = open_port(port);
  EXPECT_TRUE(fd > -1);
}

TEST(XbeeIOTest, openPort2){
  std::string port = "blah";
  int fd = open_port(port);
  EXPECT_TRUE(fd == -1);
}

TEST(XbeeIOTest, setupPort1){
  std::string port = "/dev/ttyUSB0";
  int fd = open_port(port);
  EXPECT_TRUE(setup_port(fd, 57600, 0, 100, true, true));
}

TEST(XbeeIOTest, setupPort2){
  std::string port = "/dev/ttyUSB0";
  int fd = open_port(port);
  EXPECT_FALSE(setup_port(-1, 57600, 0, 100, true, true));
}

TEST(XbeeIOTest, closePort1){
  std::string port = "/dev/ttyUSB0";
  int fd = open_port(port);
  EXPECT_TRUE(close_port(fd));
}
